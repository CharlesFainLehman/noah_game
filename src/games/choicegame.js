...
import { WIN, FEET, GOOD, BAD, drawStars, drawMessage } from './numgame.js';
...
export function choiceGame({ game, gen, bg, who, title, onDone }) {
  ...
  function draw() {
    ...
    px.blit(pixelCharacter(who, expr), 2, FEET[bg] - CH * 2, 2);
    drawStars(px, wins);
    // Card
    ...
    if (state === 'ask') { px.box(60, 84, 200, 20, '#fffdf5'); px.text(hintOn && q.hint ? q.hint : q.prompt || 'TAP THE RIGHT ONE', 160, 90, P.k, { align: 'center' }); }
