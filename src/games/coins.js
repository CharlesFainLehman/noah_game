...
export function coinPurse(loc, onDone) {
  ...
  function draw() {
    px.clearHits();
    drawBackdrop(px, loc.bg);
    const expr = state === 'bad' ? 'sad' : (state === 'good' || state === 'done') ? 'happy' : 'normal';
    px.blit(pixelCharacter(loc.who, expr), 2, FEET[loc.bg] - CH * 2, 2);
    drawStars(px, wins);
    ...
    if (q.kind === 'count') {
      ...
      if (state === 'ask') { px.box(60, 84, 200, 20, '#fffdf5'); px.text(reveal ? 'HOW MANY CENTS?' : 'TAP THE CARD FOR HELP', 160, 90, P.k, { align: 'center' }); }
      ...
      if (state === 'ask') { px.box(60, 84, 200, 20, '#fffdf5'); px.text('TAP COINS TO PAY THE PRICE', 160, 90, P.k, { align: 'center' }); }
      else drawMessage(px, state, msg, levelMsg);
      // Coin tray
      info.tray = [];
      TYPES.forEach((type, i) => {
        const x = 20 + i * 60, y = 118;
        px.box(x, y, 52, 30, P.tan);
      ...
      px.box(60, 152, 84, 22, '#ffd6d6'); px.text('CLEAR', 102, 159, P.k, { align: 'center' });
      px.hit(60, 152, 84, 22, () => { picked = []; sfx.tap(); });
      px.box(176, 152, 84, 22, '#9be07d'); px.text('PAY', 218, 159, P.k, { align: 'center' });
      px.hit(176, 152, 84, 22, () => { if (picked.length) check(total); });
      info.pads = { clear: { x: 60, y: 152, w: 84, h: 22 }, serve: { x: 176, y: 152, w: 84, h: 22 } };
    }
    if (state === 'done') { px.box(60, 30, 200, 40, P.yellow); px.text('ALL DONE!', 160, 40, P.k, { align: 'center', scale: 2 }); }
    window.__game = info;
  }
